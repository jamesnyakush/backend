...
LABEL maintainer="James Nyakundi"
...
RUN mkdir bin/
RUN go build -o bin/nyumbapoa-server cmd/nyumbapoa-server.go
...
RUN mkdir bin/
COPY --from=go-builder /go/src/application/bin/nyumbapoa-server ./bin

# expose the port that the server starts on
EXPOSE 6700

CMD ["./bin/nyumbapoa-server"]
